...
const jackpotLevels = db.levels.findAll()
.then((data) => data)
...
const triggerSpin = async (client_server, bet) => {
  ...
  const canWinJackpot = jackpotLevels.map(level => {
    return level.seed >= level.lowerBoundary && level.displayedAmount <= level.actualAmount && probability(probabilityForJackpot);
  });
  const hasWonJackpot = canWinJackpot.indexOf(true); 
  const wonClient = probability(client.probabilityForWin/100) &&  bets[bet] <= client.moneyIn; //set in db 
...
const distributeTheBet = (bet) => {
  ...
  jackpotLevels.forEach(async level => {//check if it edits the array
    level.seed += addToPot;
...
const addBetToClientPot = async (client_id, bet) => {
  const client = await db.clients.find({where: {uuid: client_id}});
  client.moneyIn += bet;
  client.save();
  return client;
}
